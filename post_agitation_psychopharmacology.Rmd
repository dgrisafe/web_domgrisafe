...
names(df_agitate_pretty) = c("Generic", "Brand", "Initial Dose (mg)", "Max Total Daily Dose (mg)", "Time to Max Concentration (min)", "Time Before Repeat Administration (min)")

# print html table
df_agitate_pretty %>% kbl(escape = F, align=c(rep('l', 2), rep('c', 4))) %>% kable_styling() %>% 
  group_rows(start_row = 1, end_row = 4, "Oral (PO)", bold = F) %>% 
  group_rows(start_row =5, end_row = 10, "Intramuscular (IM)", bold = F) %>% 
  group_rows(start_row =11, end_row = 11, "Intravenous (IV)", bold = F) %>% 
  footnote(symbol = c(
...
<center><img src="images/agitation_psychopharmacology.png" width=50%></img></center>
<p style="clear:both;"><hr></p>
